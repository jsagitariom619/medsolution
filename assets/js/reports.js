...
function setup(){document.getElementById('referenceDate').value=new Date().toISOString().slice(0,10);load().catch(e=>alert(e.message));['periodFilter','referenceDate','serviceFilter','responsibleFilter','userFilter'].forEach(id=>document.getElementById(id).addEventListener('change',render));document.getElementById('printReportBtn').onclick=()=>window.print();window.MedSolutionData.subscribeAttentions(()=>load())}
